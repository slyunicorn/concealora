...
# | Concept | Implementation in `covert\_lora.m` |

# |---|---|

# | CSS chirp | `exp(1j\*2\*pi\*(f0\*t + (k\_chirp/2)\*t.^2))` built per symbol. |

# | Symbol encoding | `mod(ascii\_val, 2^SF)` then a cyclic shift `exp(1j\*2\*pi\*(sym\*BW/2^SF)\*t)`. |

# | \*\*Parameter hopping\*\* | `SF\_seq`, `BW\_seq`, `freq\_offset\_seq` drawn per symbol with a seeded RNG — this sequence \*is\* the shared key. |

# | Covert frequency offset | An extra `exp(1j\*2\*pi\*freq\_offset\*t)` term (±5 kHz) that further smears the signal off the nominal channel center. |

# | Channel | `pathloss = @(d) (4\*pi\*d\*fc/c).^2` plus complex AWGN scaled by a single `noise\_power`. |
...
# | \*\*Detection analysis (LPD)\*\* | A \*\*channelized energy detector\*\* (`band\_energy`) integrates energy only inside a standard 125 kHz LoRa channel; `collect\_stats` runs a Monte-Carlo sweep to build ROC / Pd curves. |
...
# &#x20;   exportgraphics(f, sprintf('figures/fig\_%02d.png', f.Number), 'Resolution', 150);
...
# !\[Spectrogram comparison](figures/fig\_01.png)
...
# !\[Time domain comparison](figures/fig\_02.png)
...
# !\[Parameter hopping](figures/fig\_03.png)
...
# !\[Distance vs SNR](figures/fig\_04.png)
...
# !\[SNR vs success](figures/fig\_05.png)
...
# !\[Accuracy by SF](figures/fig\_06.png)
...
# !\[Energy detector ROC](figures/fig\_07.png)
...
# !\[Pd vs eavesdropper distance](figures/fig\_08.png)
...
# !\[TX power vs Pd](figures/fig\_09.png)
...
# ├── covert\_lora.m      % full simulation: TX, channel, RX, baseline, detector, all 9 figures
